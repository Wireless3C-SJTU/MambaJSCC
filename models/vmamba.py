...
class Adaptive_SelectiveScan(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, nrows=1,snr=10):
        ...
        if SSMODE == "mamba_ssm":
            ...
            out, x, *rest = adaptive_selective_scan_cuda_core.fwd(u, delta, A, B, C, D, delta_bias, delta_softplus, nrows,snr)
...
def cross_selective_scan(
    ...
    def selective_scan(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=True, nrows=1,adaptive='no',snr=10):
        #print("u.shape[2]", u.shape[2])
        if adaptive=='ssm':
            return Adaptive_SelectiveScan.apply(u, delta, A, B, C, D, delta_bias, delta_softplus, nrows,snr)
        else:
...
class SS2D(nn.Module):
    ...
    def forward_corev2(self, x: torch.Tensor, nrows=-1, SNR=10,channel_first=False):
        ...
        x = cross_selective_scan(
            ...
            nrows=nrows, delta_softplus=True,scan=self.scan,scan_number=self.scan_number,adaptive=self.adaptive,snr=SNR)
...
class conv_scan(nn.Module): 
    def __init__(self,C,num_expand,kernel=4,stride=4,padding=0):
        super().__init__()
        self.scan=nn.Conv2d(C,C*num_expand,kernel_size=kernel,stride=stride,padding=padding,groups=C)
...
class conv_merge(nn.Module):
    def __init__(self,C,num_expand,kernel=3,stride=1,padding=1,):
        super().__init__()
        self.merge=nn.Conv2d(C*num_expand,C,kernel_size=kernel,stride=stride,padding=padding,groups=C)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if extent=='no':
            ...
            self.extent = Mlp(hidden_dim, hidden_dim, hidden_dim, act_layer=mlp_act_layer, drop=mlp_drop_rate, channels_first=False)
        else:
